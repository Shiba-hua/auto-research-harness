...
所有数字为目标机实测（`evidence/bench.jsonl` / `acceptance.jsonl`）。
同一行内的基线/实验组使用**同一模型、同一框架、同一上下文长度、同一张卡**。

### A. Qwen3.8-27B（dense, AWQ-INT4）· vLLM 0.25.0 · **256k**

| 配置 | short | mid（稳态） | long(3.4k) | 聚合 | 加速比 | 接受率 |
|---|---|---|---|---|---|---|
| 基线 + CUDA graph | 41.47 | 42.90 | 20.08 | 39.33 | — | — |
| **原生 MTP** | 39.57 | **64.59** | 23.53 | **45.81** | 稳态 **1.51×** / 聚合 1.16× | **53.5%** |

### B. Qwen3.8-27B（GGUF UD-Q4_K_M）· llama.cpp CUDA · 32k

| 配置 | short | mid（稳态） | long(3.4k) | 聚合 | 加速比 | 接受率 |
|---|---|---|---|---|---|---|
| 基线 | 42.11 | 41.52 | 15.42 | 37.59 | — | — |
| **DFlash2** | 58.96 | **69.60** | 10.69 | **48.12** | 稳态 **1.68×** / 聚合 1.28× | **49.9%**（avg len 2.49） |

### C. Qwen3.6-35B-A3B（MoE, FP8）· vLLM 0.25.0 · **256k**

| 配置 | short | mid（稳态） | long(3.4k) | 聚合 | 加速比 | 接受率 |
|---|---|---|---|---|---|---|
| 基线 | 101.25 | 102.67 | 86.01 | 98.31 | — | — |
| DSpark | 115.88 | 106.72 | 73.38 | 100.75 | 1.02×（几乎无加速） | — |
| **原生 MTP** | 80.41 | **178.46** | **156.55** | **117.74** | 稳态 **1.74×** / 聚合 **1.20×** | **65.3%** |

### D. Qwen3.6-35B-A3B（MoE, GGUF UD-Q3_K_M）· llama.cpp CUDA · 32k

| 配置 | short | mid（稳态） | long(3.4k) | 聚合 | 加速比 | 接受率 |
|---|---|---|---|---|---|---|
| 基线 | 147.00 | 144.68 | 33.47 | 116.77 | — | — |
| **DFlash** | 192.59 | **180.73** | **107.18** | **162.44** | 聚合 **1.39×** / 长提示 **3.20×** | **64.0%**（avg len 2.90） |

### 汇总：哪种技术在哪最好

| 模型 | 最好技术 | 框架 | 稳态加速 |
|---|---|---|---|
| Qwen3.8-27B | **DFlash2** | llama.cpp | 1.68× |
| | MTP | vLLM | 1.51× |
| Qwen3.6-35B-A3B (MoE) | **MTP** | vLLM | 1.74× |
| | DFlash | llama.cpp | 1.25×（但长提示 3.20×） |
| | DSpark | vLLM | 1.04×（无效） |
...
| 组合 | 原因 |
|---|---|
| 27B × DSpark (RadixArk) | vLLM 不兼容：该实现绑定 DeepSeek-V4 的 `hc_mult`/`hc_eps`/`hc_sinkhorn_iters` 超连接 |
| 27B × DFlash (v1) | z-lab 未发布 Qwen3.8-27B 的 DFlash v1（只发了 DFlash2） |
| MoE × DFlash2 | 无对应草稿发布（MoE 只有 DFlash v1） |

### 两点值得强调的观察

1. **同一模型换一种草稿，结论可以完全不同。** DSpark 在 MoE 上只有 1.02×，
   而 MTP 在同一模型同一框架上给出 1.74×。所以"MoE 推测解码收益小"是错的 ——
   是那个草稿不行，不是 MoE 不行。
2. **长上下文场景收益更大。** MoE 的 DFlash 在长提示上 33.47 → 107.18（3.20×），
   远超其聚合值。推测解码在 prefill 占比高的场景价值被低估了。
